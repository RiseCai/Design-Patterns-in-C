...
PLANTUML_JAR = os.path.join(PROJECT_ROOT, 'plantuml-latest.jar')

def generate_svg(puml_file, svg_file=None):
    """
    Generate SVG from PlantUML file using latest plantuml-latest.jar
    """
    if not os.path.exists(PLANTUML_JAR):
        print(f'Error: plantuml-latest.jar not found at {PLANTUML_JAR}')
        return False
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print(f'Successfully generated SVG for {puml_file}')
...
def main():
    ...
    print(f'Successfully generated {success_count}/{len(puml_files)} SVG files.')
